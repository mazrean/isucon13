...
func postLivecommentHandler(c echo.Context) error {
	...
	// スパム判定
	var checker int
	if err := tx.SelectContext(ctx, &checker, "SELECT 1 FROM ng_words WHERE user_id = ? AND livestream_id = ? AND ? LIKE CONCAT('%', word, '%')", livestreamModel.UserID, livestreamModel.ID, req.Comment); err != nil && errors.Is(err, sql.ErrNoRows) {
